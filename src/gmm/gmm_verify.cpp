...
float verify(const GMM &gmm_candidate, const fmatrix &dat)
{
	...
	/// \FET Hem implementat la verificació de la puntuació basada en el gmm del candidat
...
float verify(const GMM &gmm_candidate, const GMM & gmm_world, const fmatrix &dat, float &lprobcand, float &lprobbackground)
{
	 ...
	 */
	/// \FET hem implementat la puntuació de verificació basada en el gmm del candidat i el model 'world'
	float score = 0.0F;
	lprobcand = 0.0F;
